...
tf_x = img_src[np.newaxis, :, :, :].astype(np.float32).transpose((0, 3, 1, 2))
tf_y = tf_bilinear(tf_x).transpose((0, 2, 3, 1))[0]

np_x = img_src.astype(np.float32)
np_y = np_bilinear(np_x)

print('diff:%f' % np.sum(np.abs(tf_y - np_y)))

cv2.imwrite('testimg/test_bilinear_NCHW.png', tf_y)